...
USER_HEADER = """/************************************************************
  songs_user.h - your own three-voice track
  GENERATED by tools/import_song.py from {source}
  Regenerate, or edit by hand - either is fine.

  Track: {title} ({style}), {bpm} BPM
  Length: {bars:g} bars, {one_pass:.1f} s per pass, x{passes} = {total:.1f} s

  Voice lengths (all three must agree, and do):
{voice_report}
************************************************************/
...
#define HAVE_USER_SONG 1

#define USER_SONG_TITLE "{title}"
#define USER_SONG_STYLE "{style}"
#define USER_SONG_BPM   {bpm}
...
def install(arrays, names, args, source):
    """Write sketch/songs_user.h with the track enabled."""
    here = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    out = os.path.join(here, "sketch", "songs_user.h")

    slots = ["userLead", "userHarm", "userBass"]
    ...
    report = "\n".join(
        f"    {labels[i]:<8} {names[i]:<16} {float(lens[i]):g} bars, "
        f"{len(arrays[names[i]])} notes"
        for i in range(len(names)))

    body = [USER_HEADER.format(
        source=os.path.basename(source), title=args.title, style=args.style,
        bpm=args.bpm, bars=bars, one_pass=one_pass, passes=passes,
        total=one_pass * passes, voice_report=report)]

    for i, n in enumerate(names):
        body.append(f"// ---- Voice {i + 1}: {labels[i]} "
                    f"(imported as {n}) ----")
        body.append(emit_array(slots[i], arrays[n]))
        body.append("")

    # Any slot the source did not fill stays silent.
    for i in range(len(names), 3):
        body.append(f"// ---- Voice {i + 1}: {labels[i]} - not supplied ----")
        body.append(f"const int {slots[i]}[] = {{ REST,1 }};")
        body.append("")

    body.append("#endif  // HAVE_USER_SONG\n")
    body.append("#endif  // SONGS_USER_H")

    with open(out, "w") as f:
        f.write("\n".join(body) + "\n")

    print(f"\ninstalled \"{args.title}\" -> {out}")
    print(f"  {bars:g} bars at {args.bpm} BPM = {one_pass:.1f} s per pass, "
          f"x{passes} = {one_pass * passes:.1f} s")
    print("  HAVE_USER_SONG is now 1; the track is last in the playlist.")
    print("\nnext:")
    print("  cd tools && make preview     # renders it to ../preview/*.wav")
    print("  cd tools && make verify      # checks its timing like any track")
    return 0
